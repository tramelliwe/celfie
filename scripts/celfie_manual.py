...
def celfie_main(input_path,output_directory,num_samples,max_iterations=1000,
                unknowns=0,parallel_job_id=1,convergence=0.0001,
                arg_random_restarts=10):
    ...
    output_gamma_file = f"{output_directory}/{parallel_job_id}_methylation_proportions.txt"
    
    print(f"beginning generation of {output_directory}")
    print()
    
    # make input arrays and add the specified number of unknowns
    x, x_depths, y, y_depths = define_arrays(data_df, int(num_samples), int(unknowns))
    
    # get header for output files
    samples, tissues = get_header(data_df, num_samples, unknowns)
    
    # Run EM with the specified iterations and convergence criteria
    random_restarts = []
    
    for i in range(arg_random_restarts):
        alpha, gamma, ll = em(
            x, x_depths, y, y_depths, max_iterations, convergence
        )
